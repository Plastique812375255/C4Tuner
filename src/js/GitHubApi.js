...
GitHubApi.prototype.getFileLastCommitInfo = function (
  project,
  branch,
  filename,
  callback,
  ...
  $.getJSON(
    `${self.GITHUB_API_URL}repos/${encodeURI(project)}/commits?sha=${encodeURIComponent(branch)}&path=${encodeURIComponent(filename)}`,
    function (commits) {
      const result = {};
      try {
